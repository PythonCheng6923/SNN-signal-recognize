...
path = r'D:\pythonProject\Python Workspace\SNN信号调制识别\任务代码和数据集\dataset\任务\train'
...
for file_name in file_list:
    ...
    if os.path.isfile(file_path):
        with open(file_path, "rb") as f:
            ...
            x_train.append(IQ_samples_data)
            y_train.append(IQ_labels_array)
            snrs_train.append(IQ_snrs_array)
...
snrs_train = np.concatenate(snrs_train)


print("总x_train的尺寸：", x_train.shape)
print("总y_train的尺寸：", y_train.shape)
print("总snrs_train的尺寸：", snrs_train.shape)

# save
if not(os.path.exists(save_path + "/data")):
    os.mkdir(save_path + "/data")

np.save(save_path + "/data/x_train.npy", x_train)
np.save(save_path + "/data/y_train.npy", y_train)
np.save(save_path + "/data/snrs_train.npy", snrs_train)
